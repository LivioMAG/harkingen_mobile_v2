...
const elements = {
  ...
  openWeeklyImportBtn: document.getElementById('openWeeklyImportBtn'),
  weeklyImportPanel: document.getElementById('weeklyImportPanel'),
  cancelWeeklyImportBtn: document.getElementById('cancelWeeklyImportBtn'),
  weeklyImportFileInput: document.getElementById('weeklyImportFileInput'),
  startWeeklyImportBtn: document.getElementById('startWeeklyImportBtn'),
  weeklyImportMeta: document.getElementById('weeklyImportMeta'),
  weeklyImportWarnings: document.getElementById('weeklyImportWarnings'),
  weeklyImportPreview: document.getElementById('weeklyImportPreview'),
  weeklyImportPreviewTitle: document.getElementById('weeklyImportPreviewTitle'),
  weeklyImportPreviewSubtitle: document.getElementById('weeklyImportPreviewSubtitle'),
  weeklyImportPreviewBody: document.getElementById('weeklyImportPreviewBody'),
  confirmAllWeeklyImportBtn: document.getElementById('confirmAllWeeklyImportBtn'),
...
function getSavedCommissionNumbers() {
  return (state.profile?.saved_commissions || [])
    .map((commission) => String(commission.number || commission.commission_number || '').trim())
    .filter(Boolean);
}

function findSavedProjectName(commissionNumber) {
  const match = (state.profile?.saved_commissions || []).find((commission) => (
    String(commission.number || commission.commission_number || '').trim() === String(commissionNumber || '').trim()
  ));
  return match?.project || match?.project_name || '';
}
...
function getWeeklyImportEntries() {
  return state.weeklyImportBatch?.entries || [];
}

function resetWeeklyImport() {
  state.weeklyImportBatch = null;
  if (elements.weeklyImportFileInput) elements.weeklyImportFileInput.value = '';
  elements.weeklyImportMeta?.classList.add('hidden');
  elements.weeklyImportWarnings?.classList.add('hidden');
  elements.weeklyImportPreview?.classList.add('hidden');
  if (elements.weeklyImportPreviewBody) elements.weeklyImportPreviewBody.innerHTML = '';
}

function openWeeklyImportPanel() {
  elements.weeklyImportPanel?.classList.remove('hidden');
  elements.weeklyImportFileInput?.focus();
  refreshIcons(elements.weeklyImportPanel);
}

function closeWeeklyImportPanel() {
  resetWeeklyImport();
  elements.weeklyImportPanel?.classList.add('hidden');
}

function entryHasDuplicateWarning(entry) {
  return (entry.warnings || []).some((warning) => /duplikat/i.test(warning));
}

function isImportEntryConfirmable(entry) {
  return entry?.status === 'pending' && !entryHasDuplicateWarning(entry);
}
...
function renderWeeklyImportPreview() {
  const batch = state.weeklyImportBatch;
  if (!batch || !elements.weeklyImportPreviewBody) return;
  const entries = getWeeklyImportEntries();
  ...
  const confirmableCount = entries.filter(isImportEntryConfirmable).length;

  elements.weeklyImportMeta.innerHTML = `
    <span class="pill neutral">Datei: ${escapeHtml(batch.fileName)}</span>
    <span class="pill neutral">KW ${escapeHtml(String(batch.calendarWeek))}/${escapeHtml(String(batch.year))}</span>
    <span class="pill neutral">${escapeHtml(batch.employeeName || 'Mitarbeiter unbekannt')}</span>
  `;
  elements.weeklyImportMeta.classList.remove('hidden');
  elements.weeklyImportPreviewTitle.textContent = `${entries.length} Zeiteinträge gefunden`;
  elements.weeklyImportPreviewSubtitle.textContent = `${pendingCount} offen (${confirmableCount} bestätigbar), ${confirmedCount} bestätigt, ${rejectedCount} abgelehnt. Spesen, Auto, Mofa, Bemerkungen und Total-Spalten wurden ignoriert.`;
  elements.confirmAllWeeklyImportBtn.disabled = confirmableCount === 0;

  const allWarnings = [...(batch.warnings || [])];
  entries.forEach((entry) => (entry.warnings || []).forEach((warning) => allWarnings.push(`Zeile ${entry.sourceRow}: ${warning}`)));
  if (allWarnings.length) {
    elements.weeklyImportWarnings.innerHTML = allWarnings.map((warning) => `<div>${escapeHtml(warning)}</div>`).join('');
    elements.weeklyImportWarnings.classList.remove('hidden');
  } else {
    elements.weeklyImportWarnings.classList.add('hidden');
    elements.weeklyImportWarnings.innerHTML = '';
  }

  elements.weeklyImportPreviewBody.innerHTML = '';
  entries.forEach((entry) => {
    const row = document.createElement('tr');
    row.className = `import-row import-row-${entry.status}`;
    const warnings = (entry.warnings || []).length ? `<div class="import-entry-warning">${entry.warnings.map(escapeHtml).join('<br>')}</div>` : '';
    const confirmable = isImportEntryConfirmable(entry);
    row.innerHTML = `
      <td>${escapeHtml(formatDate(parseLocalDate(entry.date)))}</td>
      <td>${escapeHtml(entry.weekday)}</td>
      <td>${escapeHtml(entry.commissionNumber)}</td>
      <td>${escapeHtml(entry.projectName || '—')}${warnings}</td>
      <td>${escapeHtml(formatHours(entry.hours))}</td>
      <td><span class="pill ${entry.status === 'confirmed' ? 'success' : entry.status === 'rejected' ? 'danger' : 'warning'}">${getImportStatusLabel(entry.status)}</span></td>
      <td class="import-actions-cell">
        <button class="secondary-btn small" data-import-confirm="${entry.id}" type="button" ${!confirmable ? 'disabled' : ''}>Bestätigen</button>
        <button class="ghost-btn small" data-import-reject="${entry.id}" type="button" ${entry.status !== 'pending' ? 'disabled' : ''}>Ablehnen</button>
      </td>
    `;
    elements.weeklyImportPreviewBody.appendChild(row);
  });
  elements.weeklyImportPreview.classList.remove('hidden');
  refreshIcons(elements.weeklyImportPreview);
}

async function readWeeklyImportFile(file) {
  if (!file) throw new Error('Bitte eine XLSX-Datei auswählen.');
  const lowerName = file.name.toLowerCase();
  if (!lowerName.endsWith('.xlsx')) throw new Error('Dieses Format wird noch nicht unterstützt. Bitte XLSX hochladen.');
  if (!window.XLSX) throw new Error('XLSX-Bibliothek konnte nicht geladen werden.');
  const buffer = await file.arrayBuffer();
  const workbook = window.XLSX.read(buffer, { type: 'array', cellDates: false });
  return window.WeeklyReportImport.parseWorkbook(workbook, {
    XLSX: window.XLSX,
    knownCommissions: getSavedCommissionNumbers()
  });
}

async function startWeeklyImport() {
  if (!state.supabase || !state.session?.user) {
    showToast('Bitte zuerst anmelden.', 'error');
    return;
  }
  const file = elements.weeklyImportFileInput?.files?.[0];
  try {
    await runWithFeedback(
      {
        button: elements.startWeeklyImportBtn,
        section: elements.weeklyImportPanel,
        pendingMessage: 'Wochenrapport wird analysiert …',
        loadingLabel: 'Analysieren …'
      },
      async () => {
        const parsed = await readWeeklyImportFile(file);
        const preparedEntries = window.WeeklyReportImport.markDuplicateWarnings(parsed.entries, state.entries).map((entry) => ({
          ...entry,
          projectName: entry.projectName || findSavedProjectName(entry.commissionNumber)
        }));
        const batchResult = await state.supabase
          .from('weekly_report_import_batches')
          .insert({
            profile_id: state.session.user.id,
            source_file_name: file.name,
            employee_name: parsed.employeeName || '',
            calendar_week: parsed.calendarWeek,
            year: parsed.year,
            warnings: parsed.warnings || []
          })
          .select('id, source_file_name, employee_name, calendar_week, year, status, warnings')
          .single();
        if (batchResult.error) throw batchResult.error;
        const importBatchId = batchResult.data.id;
        const previewRows = preparedEntries.map((entry) => ({
          import_batch_id: importBatchId,
          profile_id: state.session.user.id,
          commission_number: entry.commissionNumber,
          project_name: entry.projectName || '',
          work_date: entry.date,
          weekday: entry.weekday,
          hours: entry.hours,
          employee_name: entry.employeeName || '',
          calendar_week: entry.calendarWeek,
          year: entry.year,
          status: 'pending',
        ...
        const entriesResult = await state.supabase
          .from('weekly_report_import_entries')
          .insert(previewRows)
          .select('id, import_batch_id, commission_number, project_name, work_date, weekday, hours, employee_name, calendar_week, year, status, warnings, source_row');
        if (entriesResult.error) throw entriesResult.error;
        const entriesWithWarnings = entriesResult.data.map((row, index) => ({
          ...preparedEntries[index],
          id: row.id,
          importEntryId: row.id,
          importBatchId: row.import_batch_id,
          sourceFileId: row.import_batch_id,
          status: row.status,
          warnings: row.warnings || preparedEntries[index].warnings || []
        }));
        state.weeklyImportBatch = {
          ...parsed,
          id: importBatchId,
          fileName: batchResult.data.source_file_name,
          employeeName: batchResult.data.employee_name || parsed.employeeName,
          calendarWeek: batchResult.data.calendar_week || parsed.calendarWeek,
          year: batchResult.data.year || parsed.year,
          warnings: batchResult.data.warnings || parsed.warnings || [],
          entries: entriesWithWarnings
        };
        renderWeeklyImportPreview();
        showToast('Import-Preview erstellt.', 'success');
      }
    );
  } catch (error) {
    console.error(error);
...
async function confirmWeeklyImportEntry(entryId) {
  const entry = getWeeklyImportEntries().find((item) => item.id === entryId);
  if (!entry || !isImportEntryConfirmable(entry)) return;
  if (!state.supabase || !state.session?.user) {
    showToast('Bitte zuerst anmelden.', 'error');
    return;
  }
    ...
    await runWithFeedback(
      {
        button: null,
        section: elements.weeklyImportPanel,
        pendingMessage: 'Import-Eintrag wird gespeichert …',
        loadingLabel: 'Speichern …'
      },
      async () => {
        const payload = window.WeeklyReportImport.buildWeeklyReportPayload(entry, state.session.user.id);
        ...
        const updateResult = await state.supabase
          .from('weekly_report_import_entries')
          .update({ status: 'confirmed', weekly_report_id: result.data?.id || null })
          .eq('id', entry.importEntryId || entry.id)
          .eq('profile_id', state.session.user.id);
        if (updateResult.error) throw updateResult.error;
        entry.status = 'confirmed';
        entry.savedReportId = result.data?.id;
        await loadEntries();
        renderWeek();
        renderWeeklyImportPreview();
        showToast('Import-Eintrag bestätigt.', 'success');
      }
    );
  } catch (error) {
    console.error(error);
    showToast(`Bestätigung fehlgeschlagen: ${error.message}`, 'error');
  }
}

function rejectWeeklyImportEntry(entryId) {
  const entry = getWeeklyImportEntries().find((item) => item.id === entryId);
  if (!entry || entry.status !== 'pending') return;
  if (state.supabase && state.session?.user && entry.importEntryId) {
    state.supabase
      .from('weekly_report_import_entries')
      .update({ status: 'rejected' })
      .eq('id', entry.importEntryId)
      .eq('profile_id', state.session.user.id)
      .then(({ error }) => {
        if (error) {
          console.error(error);
          showToast(`Ablehnen konnte nicht gespeichert werden: ${error.message}`, 'error');
        }
      });
  }
...
async function confirmAllWeeklyImportEntries() {
  const pendingEntries = getWeeklyImportEntries().filter(isImportEntryConfirmable);
  ...
  try {
    await runWithFeedback(
      {
        button: elements.confirmAllWeeklyImportBtn,
        section: elements.weeklyImportPanel,
        pendingMessage: 'Alle Import-Einträge werden gespeichert …',
        loadingLabel: 'Speichern …'
      },
      async () => {
        const payloads = pendingEntries.map((entry) => window.WeeklyReportImport.buildWeeklyReportPayload(entry, state.session.user.id));
        const result = await state.supabase
          .from('weekly_reports')
          .insert(payloads)
          .select(WEEKLY_REPORT_COLUMNS);
        if (result.error) throw result.error;
        const updateRows = pendingEntries.map((entry, index) => ({
          id: entry.importEntryId || entry.id,
          status: 'confirmed',
          weekly_report_id: result.data?.[index]?.id || null
        }));
        const updateResults = await Promise.all(updateRows.map((row) => state.supabase
          .from('weekly_report_import_entries')
          .update({ status: row.status, weekly_report_id: row.weekly_report_id })
          .eq('id', row.id)
          .eq('profile_id', state.session.user.id)));
        const updateError = updateResults.find((updateResult) => updateResult.error)?.error;
        if (updateError) throw updateError;
        pendingEntries.forEach((entry, index) => {
          entry.status = 'confirmed';
          entry.savedReportId = result.data?.[index]?.id;
        });
        await loadEntries();
        renderWeek();
        renderWeeklyImportPreview();
        showToast('Alle gültigen Import-Einträge bestätigt.', 'success');
      }
    );
  } catch (error) {
    console.error(error);
    showToast(`Alle bestätigen fehlgeschlagen: ${error.message}`, 'error');
  }
}
...
function registerEventListeners() {
  ...
  elements.openWeeklyImportBtn?.addEventListener('click', openWeeklyImportPanel);
  elements.cancelWeeklyImportBtn?.addEventListener('click', closeWeeklyImportPanel);
  elements.startWeeklyImportBtn?.addEventListener('click', startWeeklyImport);
  elements.confirmAllWeeklyImportBtn?.addEventListener('click', confirmAllWeeklyImportEntries);
  elements.weeklyImportPreviewBody?.addEventListener('click', (event) => {
    const confirmTarget = event.target.closest('[data-import-confirm]');
    const rejectTarget = event.target.closest('[data-import-reject]');
    if (confirmTarget) {
      confirmWeeklyImportEntry(confirmTarget.dataset.importConfirm);
      return;
    }
    if (rejectTarget) rejectWeeklyImportEntry(rejectTarget.dataset.importReject);
  });
